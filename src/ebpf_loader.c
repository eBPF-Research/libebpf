...
int
ubpf_load_elf(struct ebpf_vm* vm, const void* elf, size_t elf_size, char** errmsg)
{
    ...
    for (i = 0; i < ehdr->e_shnum; i++) {
        ...
        for (j = 0; j < rel->size / sizeof(Elf64_Rel); j++) {
            ...
            case R_BPF_64_64: {
                ...
                struct bpf_insn* inst = (struct bpf_insn*)(text_copy + r.r_offset);
                struct bpf_insn* inst2 = (struct bpf_insn*)(text_copy + r.r_offset + sizeof(struct bpf_insn));
                if (inst->code != EBPF_OP_LDDW) {
                    *errmsg = ubpf_error("bad relocation instruction");
                    goto error;
                }
                if (r.r_offset + sizeof(struct bpf_insn) * 2 > text->size) {
                    *errmsg = ubpf_error("bad relocation offset");
                    goto error;
                }

                uint64_t imm = do_data_relocation(
                    NULL, map->data, map->size, sym_name, sym.st_value, sym.st_size);
