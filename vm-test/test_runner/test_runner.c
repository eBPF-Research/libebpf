...
int main(int argc, char **argv) {
    ...
    struct ebpf_vm *vm = ebpf_vm_create();
    if (!vm) {
...
#endif
        rv = ebpf_vm_load_instructions(vm, code, code_len / 8);
#if defined(UBPF_HAS_ELF_H)
    }
#endif
    if (unload) {
        ebpf_vm_unload_instructions(vm);
        unload = false;
        goto load;
    }
    if (reload) {
...
uint64_t memfrob_ext(uint64_t s, uint64_t n, uint64_t _a, uint64_t _b, uint64_t _c) {
    size_t p1 = s;
    for (int i = 0; i < n; i++) {
        ((char *)p1)[i] ^= 42;
    }
...
static uint64_t gather_bytes(uint64_t a, uint64_t b, uint64_t c, uint64_t d, uint64_t e) {
    return (((uint64_t)a) << (uint64_t)32) | (((uint32_t)b) << (uint64_t)24) | (((uint32_t)c) << (uint64_t)16) | (((uint16_t)d) << (uint64_t)8) |
           (uint64_t)e;
}

static uint64_t trash_registers(uint64_t _a, uint64_t _b, uint64_t _c, uint64_t _d, uint64_t _e) {
...
static uint64_t sqrti(uint64_t x, uint64_t _a, uint64_t _b, uint64_t _c, uint64_t _d) {
    return (uint32_t)sqrt(x);
}

static uint64_t unwind(uint64_t i, uint64_t _a, uint64_t _b, uint64_t _d, uint64_t _e) {
    return i;
}

static uint64_t strcmp_ext(uint64_t a, uint64_t b, uint64_t _c, uint64_t _d, uint64_t _e) {
    size_t p1 = a;
    size_t p2 = b;
...
static void register_functions(struct ebpf_vm *vm) {
    ebpf_vm_register_external_helper(vm, 0, "gather_bytes", gather_bytes);
    ebpf_vm_register_external_helper(vm, 1, "memfrob", memfrob_ext);
    ebpf_vm_register_external_helper(vm, 2, "trash_registers", trash_registers);
    ebpf_vm_register_external_helper(vm, 3, "sqrti", sqrti);
    ebpf_vm_register_external_helper(vm, 4, "strcmp_ext", strcmp_ext);
    ebpf_vm_register_external_helper(vm, 5, "unwind", unwind);
    // ebpf_vm_register_external_helper(vm, 5);
}
